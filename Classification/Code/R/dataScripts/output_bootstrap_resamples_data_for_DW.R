...
files= c("brcaMatchedTN","ERpn","kirckirp","luadlusc","lumAB")#,"6cancer")
...
#ids = read.table('~/OneDrive - NextCODE Health/Projects/quantumML/Classification/quantum/Data/lumAB_top44_diffexp_genes.txt')
...
splits = as.matrix(read.table(paste(block_path,f,"_bootstrap_resamples.txt",sep="")))
...
for (j in seq(st_idx,n_splits,stop_idx)) {
  ...
  #dw_test = cbind(as.numeric(class_test)-1,pc_test)
    ...
    dw_train = cbind(class_train==pos_class,x_train)
    dw_test = cbind(class_test==pos_class,x_test)
  }
