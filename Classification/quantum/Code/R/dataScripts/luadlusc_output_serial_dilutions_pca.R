...
# load data 
train_data = read_feather("/home/richard/Dropbox-Work/Wuxi/Data/data5_luadlusc_train_normalized.feather")
test_data = read_feather("/home/richard/Dropbox-Work/Wuxi/Data/data5_luadlusc_test_normalized.feather")
...
block_path = '/home/richard/Dropbox-Work/Wuxi/Data/luadlusc_splits/'
...
all_fracs = c(round(min_frac,2),all_fracs)
### Generate Splits
### Main Model Loop
for (i in 1:length(all_fracs)) {
  ...
  for (j in 10:n_splits) {
    ...
    class_exp_test = factor(c(class_test, class_valid))
    levels(class_exp_test) = levels(class_valid)
    ...
    z_train = cbind(class_train==pos_class,X_train_j_z)
    z_test = cbind(class_test==pos_class,X_test_j_z)
    z_valid = cbind(class_valid==pos_class,X_valid_j_z)
    z_exp_test = cbind(class_exp_test==pos_class,X_exp_test_j_z)
