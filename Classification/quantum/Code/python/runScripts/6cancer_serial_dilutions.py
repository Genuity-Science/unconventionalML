...
#lambdas = [0,0.5,2]
embedding = pickle.load(gzip.open(file_path + 'Embeddings/NASA_EmbedK65.pkl.gz'))
f = '6cancer'
...
for cinit in cinits :
    for frac in frac_str:
        ...
        for n in range(len(traindatas)):
            out = DW_utils.runMultiCV(traindatas[n], [0], embedding, solver_name='NASA', num_reads=1000, num_gauges=10,coupling_init=cinit,stop_point=-0.01,method='vote',annealing_time=5)
            outlist[n] = out
